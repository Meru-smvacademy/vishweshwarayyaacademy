...
export default async function AdminEnquiriesPage({
  ...
  const { q, status, program } = await searchParams;
  ...
  if (searchTerm) {
    ...
    query = query.or(
      `student_name.ilike.${pattern},parent_name.ilike.${pattern},phone.ilike.${pattern},reference_no.ilike.${pattern}`,
    );
  }
    ...
    <div className="admin-content-wrap">
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ margin: 0, fontFamily: SANS, fontSize: 14, color: "rgba(248,243,232,0.5)" }}>
          Review and manage enquiries submitted through the public admission enquiry form.
        </p>
      </div>

      <EnquiryFilters initialQuery={q ?? ""} initialStatus={status ?? ""} initialProgram={program ?? ""} />

      {error ? (
        <div style={{ border: "1px solid rgba(229,115,115,0.3)", background: "rgba(229,115,115,0.06)", padding: "24px", borderRadius: 3 }}>
          <p style={{ margin: "0 0 6px", fontFamily: SANS, fontSize: 13.5, color: "#E57373", fontWeight: 600 }}>Could not load enquiries.</p>
          <p style={{ margin: 0, fontFamily: SANS, fontSize: 12.5, color: "rgba(248,243,232,0.55)" }}>
            ...
            <code>supabase/migrations/0004_enquiries.sql</code> has been run in your Supabase project.
          </p>
        </div>
      ) : (
        <EnquiryList enquiries={data ?? []} />
      )}
